...
class Home extends React.Component {
  ...
  init = async () => {
    let that = this;
    let diachi = await agent.DieuHanh.danhSachThauPhu()
    this.setState({
      diachi: diachi
    })
  }
            ...
            >
              ...
              >
                <Input
                  type="text"
                  style={{width: '50%'}}
                  value={this.state.diachi[this.state.index].ten}
                  onChange={(e) => {
                  let diachi = this.state.diachi
                  diachi[this.state.index].ten = e.target.value
                   this.setState({
                       diachi: diachi
                     })
                   }}
                ...
                <Button type="primary"
                  onClick={() => {
                    let that = this;
                    agent.DieuHanh.capnhapthauphu(that.state.diachi[that.state.index])
                      .then(res => {
              ...
              {this.state.index !== idx && <div> - <b>{item.ten}</b></div>}
